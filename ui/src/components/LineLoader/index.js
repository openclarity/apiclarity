...
const LineLoader = ({className, done, total, title, calculatedPercentage, displayItems=false, displayPercent=false}) => {
    ...
    return (
        <div className={classnames("line-loader-container", {[className]: className})}>
            <div className={classnames("line-loader-filler", {done: percent === 100})} style={{width: `${(done === 0 && !percentageProvided) ? 0.1 : percent}%`}}></div>
