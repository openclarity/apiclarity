...
func (s *Server) PostAPIInventoryReviewIDApprovedReview(params operations.PostAPIInventoryReviewIDApprovedReviewParams) middleware.Responder {
	...
	if err := s.dbHandler.GetReviewTableFirst(&review, params.ReviewID).Error; err != nil {
	...
	if err := s.dbHandler.UpdateApprovedReview(true, params.ReviewID); err != nil {
	...
	apiID, err := s.dbHandler.GetAPIID(host, port)
	if err != nil {
	...
	if err := s.dbHandler.PutAPISpec(apiID, string(oapSpec), specInfo, database.ReconstructedSpecType); err != nil {
	...
	go func() {
		if err := s.dbHandler.SetAPIEventsReconstructedPathID(approvedReview.PathItemsReview, host, port); err != nil {
...
func (s *Server) GetAPIInventoryAPIIDSuggestedReview(params operations.GetAPIInventoryAPIIDSuggestedReviewParams) middleware.Responder {
	...
	if err := s.dbHandler.GetAPIInventoryTableFirst(&apiInfo, params.APIID).Error; err != nil {
	...
	if err := s.dbHandler.CreateReview(review); err != nil {
